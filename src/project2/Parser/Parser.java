...
public class Parser {
    ...
    public DeclSeqNode parseDeclSeq() throws ParserException {
        ...
        // first decl
        DeclNode firstDecl = this.parseDecl();
        declSeqNode.declare.add(firstDecl);
        // rest of the decl
        if (this.scanner.currentToken() == Core.INTEGER
                || this.scanner.currentToken() == Core.OBJECT) {
            DeclSeqNode restDeclSeq = this.parseDeclSeq();
            declSeqNode.declare.addAll(restDeclSeq.declare);
        }
        return declSeqNode;
    }
